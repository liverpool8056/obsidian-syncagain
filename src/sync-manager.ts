...
export class SyncManager {
  ...
  /**
   * Fires after a sync cycle completes with no upload failures and no remaining
   * dirty work. Used by the plugin to auto-advance the E2EE state machine
   * (MIGRATING → ACTIVE, MIGRATING_TO_OFF → DISABLED) on the initiator once
   * every file has been re-uploaded under the new mode.
   */
  onCycleDrained?: () => void;
  ...
  private async runFirstSyncFlow(remoteFiles: RemoteFileEntry[]): Promise<void> {
    ...
    for (const file of this.vault.getFiles()) {
      ...
      if (remote.is_encrypted && this.encryption) {
        ...
        // Mismatch in encryption state between local and remote.
        conflicts.push({
          path: file.path,
          localMtime: file.stat.mtime,
          remoteMtime: remote.last_modified,
  ...
  private async runSyncCycle(): Promise<void> {
    ...
    // Signal "queue drained" to the plugin only when this cycle did real
    // upload work and finished cleanly. Cycles where nothing was dirty must
    // not fire — otherwise idle ticks would auto-finalize a migration that
    // was paused mid-flight (e.g. by a 402 that re-queued failed paths).
    const drained =
      failedPaths.length === 0
      && this.tracker.pendingCount === 0
      && (dirty.length > 0 || deletedPaths.length > 0);
    console.debug(
      `[SyncAgain] cycle done — dirty=${dirty.length} uploaded=${dirty.length - failedPaths.length}` +
      ` failed=${failedPaths.length} deleted=${deletedPaths.length}` +
      ` pendingAfter=${this.tracker.pendingCount} encryption=${this.encryption ? "on" : "off"}` +
      ` → onCycleDrained=${drained}`,
    );
    if (drained) {
      this.onCycleDrained?.();
    }
  ...
  private async uploadLocalFile(path: string): Promise<void> {
    ...
    const plaintextHash = md5(plainData); // Used for legacy fallback or unencrypted upload
    const contentHmac = this.encryption ? await this.encryption.calculateHMAC(plainData) : undefined;

    // Skip if content is unchanged since last sync.
    // When E2EE is on, compare against contentHmac.
    // When E2EE is off, compare against md5 (plaintextHash).
    const known = this.state.files[path];
    if (this.encryption) {
      if (known?.contentHmac === contentHmac) return;
    } else {
      if (known?.md5 === plaintextHash) return;
    }
    ...
    try {
      ...
      console.debug(
        `[SyncAgain] uploading '${path}' isEncrypted=${isEncrypted}` +
        ` contentHmac=${contentHmac ? contentHmac.slice(0, 12) + "…" : "none"}` +
      ...
      try {
        ...
        // this account. Surface the one-shot callback and mark dirty so the
        // file retries on the next cycle. NEVER fall back to plaintext when
        // we were uploading ciphertext: the vault state may be ACTIVE or
        // MIGRATING server-side, in which case writing plaintext would silently
        // leak unencrypted bytes into an encrypted vault. Resolution belongs
        // to the user/admin (grant the feature, set E2EE_FOR_ALL, or roll back).
      ...
      if (contentHmac !== undefined) entry.contentHmac = contentHmac;
  ...
  private async downloadKey(key: string, remote?: RemoteFileEntry): Promise<void> {
    ...
    if (isEncrypted) {
      if (!this.encryption) {
        // Server says these bytes are ciphertext but no decryption key is
        // loaded on this device. Writing rawData would land ciphertext in the
        // user's vault. Skip; the next cycle retries once the key is loaded.
        console.warn(
      ...
      try {
        ...
        // Decryption failed (wrong key, corrupt bytes, etc). Do NOT write
        // ciphertext to the vault — skip and retry next cycle.
